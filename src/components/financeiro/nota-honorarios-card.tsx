...
export function NotaHonorariosCard({ soloExterno = false }: { soloExterno?: boolean } = {}) {
  const [tipoFormador, setTipoFormador] = useState<"registado" | "externo">(
    soloExterno ? "externo" : "registado",
  );
  ...
  const formadores = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("formadores").select("id, nome").order("nome")).data ?? [],
  });
  ...
  const formadorDet = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("formadores").select("*").eq("id", formadorId).maybeSingle()).data,
  });
  ...
  const ufcdsDisponiveis = useQuery({
    ...
    queryFn: async () => {
      ...
      const cufIds = Array.from(
        new Set((sess ?? []).map((s: any) => s.curso_ufcd_id).filter(Boolean)),
      );
      ...
      const { data: ufcds } = await supabase
        .from("ufcds")
  ...
  async function gerar() {
    ...
    if (tipoFormador === "externo") {
      if (!extNome.trim()) {
        toast.error("Nome do formador obrigatório");
        return;
      }
      ...
      if (!formadorId) {
        toast.error("Escolha um formador");
        return;
      }
      if (modo === "ufcd" && !ufcdId) {
        toast.error("Escolha uma UFCD");
        return;
      }
    }
    try {
      ...
      await exportNotaHonorariosPdf({
        ...
        ufcdId:
          tipoFormador === "registado" ? (modo === "ufcd" ? ufcdId : ufcdId || null) : undefined,
        valorHora: vh,
        ...
        destinatario:
          destNome || destNif || destMorada
            ? { nome: destNome, nif: destNif, morada: destMorada }
            : undefined,
        ...
        formadorExterno:
          tipoFormador === "externo"
            ? {
                ...
                morada: extMorada,
                codigo_postal: extCp,
                localidade: extLocalidade,
                email: extEmail,
                iban: extIban,
              }
            : undefined,
        horasAvulso:
          tipoFormador === "externo" ? parseFloat(extHoras.replace(",", ".")) || 0 : undefined,
        descricaoAvulso: tipoFormador === "externo" ? extDescricao : undefined,
        valorTotalAvulso:
          tipoFormador === "externo" && extModoValor === "total" ? vTotal : undefined,
      });
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <p className="text-sm text-muted-foreground">
          Gera um PDF de nota de honorários. Escolha um <strong>formador registado</strong> (agrega
          sessões da base de dados) ou um <strong>formador externo</strong> (prestação única, sem
          histórico). Os logótipos e dados da empresa vêm da <em>Configuração Financeira</em>.
        </p>
        ...
        {!soloExterno && (
          <div className="inline-flex rounded-md border border-input bg-background p-0.5 text-sm">
            ...
            >
              Formador externo (prestação única)
            </button>
          </div>
        )}
        ...
        {tipoFormador === "externo" ? (
          <div className="grid gap-3 md:grid-cols-4 rounded-md border p-3 bg-muted/20">
            ...
            <div className="space-y-1.5 md:col-span-4">
              ...
              <Input value={extDescricao} onChange={(e) => setExtDescricao(e.target.value)} />
            </div>
              ...
              <>
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    min="0"
                    value={extHoras}
                    onChange={(e) => setExtHoras(e.target.value)}
                  />
                </div>
                <div className="space-y-1.5">
              ...
              <>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    min="0"
                    value={extValorTotal}
                    onChange={(e) => setExtValorTotal(e.target.value)}
                  />
                </div>
              </>
            )}
            ...
            {modo === "mes" ? (
              <div className="grid gap-3 md:grid-cols-4">
                ...
                <div className="space-y-1.5 md:col-span-4">
                  ...
                  <Select
                    value={ufcdId || "__all__"}
                    onValueChange={(v) => setUfcdId(v === "__all__" ? "" : v)}
                    disabled={!formadorId}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Todas as UFCD do mês" />
                    </SelectTrigger>
                    <SelectContent>
        ...
        <div className="grid gap-3 md:grid-cols-3 items-end rounded-md border p-3 bg-muted/30">
          <div className="flex items-center gap-2 md:col-span-1">
            ...
            />
            <Label htmlFor="aplicar-iva" className="cursor-pointer">
              Acrescer IVA (recibo com IVA)
            </Label>
          </div>
          <div className="space-y-1.5 md:col-span-2">
        ...
        {(() => {
          ...
          const totalHoras = isExt
            ? horasExt
            : sessoes.reduce((a: number, s: any) => a + Number(s.horas || 0), 0);
          ...
          const ufcdSel =
            !isExt && modo === "ufcd" && ufcdId ? preview.data?.ufcdMap.get(ufcdId) : null;
          ...
          const nDoc =
            numero ||
            ((isExt ? extNome : formadorId)
              ? `NH-${numSuf}-${String(form.nome || "")
                  .replace(/\s+/g, "")
                  .slice(0, 4)
                  .toUpperCase()}`
              : "NH-…");
          ...
          return (
            <div className="mt-2 rounded-lg border-2 border-emerald-700/40 bg-emerald-50 dark:bg-emerald-950/30 p-5 shadow-sm">
              <div className="mb-3 flex items-center justify-between border-b-2 border-emerald-700/40 pb-2">
                <div>
                  <div className="text-xs uppercase tracking-wider text-emerald-900/70 dark:text-emerald-200/70">
                    Pré-visualização
                  </div>
                ...
                <div className="text-right text-xs text-emerald-900/80 dark:text-emerald-200/80">
                  <div>
                    <span className="font-semibold">Nº</span> {nDoc}
                  </div>
                  <div>
                    <span className="font-semibold">Data:</span> {fmtDate(dataEmissao)}
                  </div>
                </div>
              </div>
              <div className="grid gap-3 md:grid-cols-2 text-xs text-emerald-950 dark:text-emerald-50">
                <div>
                  <div className="font-semibold uppercase text-[10px] tracking-wider text-emerald-900/60 dark:text-emerald-200/60 mb-1">
                    Emitente
                  </div>
                  <div className="font-medium">{form.nome || "—"}</div>
                  {form.nif && <div>NIF: {form.nif}</div>}
                  {form.morada && <div>{form.morada}</div>}
                  {(form.codigo_postal || form.localidade) && (
                    <div>{`${form.codigo_postal ?? ""} ${form.localidade ?? ""}`.trim()}</div>
                  )}
                    ...
                    <tbody>
                      <tr className="border-t border-emerald-700/10">
                        <td className="px-2 py-1">
                          {extDescricao || "Prestação de serviços de formação"}
                        </td>
                        <td className="px-2 py-1 text-right">
                          {usaTotalExt && horasExt === 0 ? "—" : `${horasExt.toFixed(2)}h`}
                        </td>
                    ...
                    <tbody>
                      {sessoes.length === 0 ? (
                        <tr>
                          <td
                            colSpan={7}
                            className="text-center px-2 py-3 text-emerald-900/60 dark:text-emerald-200/60"
                          >
              ...
              <div className="mt-3 flex justify-end">
                <div className="w-full max-w-xs space-y-1 text-xs text-emerald-950 dark:text-emerald-50">
                  ...
                  {!aplicarIva || ivaPct === 0 ? (
                    <div className="flex justify-between">
                      <span>IVA:</span>
                      <span>Regime de isenção</span>
                    </div>
                  ) : (
                    <div className="flex justify-between">
                      <span>IVA ({ivaPct}%):</span>
                      <span>+ {fmtEUR(ivaVal)}</span>
                    </div>
                  )}
                  {retPct === 0 ? (
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      <span>Retenção IRS ({retPct}%):</span>
                      <span>- {fmtEUR(ret)}</span>
                    </div>
                  )}
              ...
              {observacoes && (
                <div className="mt-3 text-xs text-emerald-950 dark:text-emerald-50">
                  <div className="font-semibold uppercase text-[10px] tracking-wider text-emerald-900/60 dark:text-emerald-200/60">
                    Observações
                  </div>
                  <div>{observacoes}</div>
                </div>
              ...
              <div className="mt-3 text-[10px] italic text-emerald-900/60 dark:text-emerald-200/60">
                Pré-visualização — o PDF final inclui os logótipos e dados da empresa.
              </div>
            </div>
          );
        })()}
        ...
        <div className="flex gap-2">
          <Button
            onClick={gerar}
            disabled={busy || (tipoFormador === "registado" ? !formadorId : !extNome.trim())}
          >
